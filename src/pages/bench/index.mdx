...
<Layout>
<main class="mx-auto mt-14 w-full max-w-lg self-center p-4 md:max-w-3xl">
  ...
  for (const a of args) {
    ...
    if (k === "iters") out.iters = parseInt(v, 10);
      ...
      out.threads = out.threads =
        parseInt(v, 10) > out.threads ? out.threads : parseInt(v, 10);
...
(async function main() {
  ...
  console.log(
    `# Node ${process.version} on ${os.type()} ${os.release()} (${os.arch()})`
  );
  console.log(
    `# logical CPUs: ${os.cpus().length}; model: ${
      os.cpus()[0]?.model || "unknown"
    }`
  );
  console.log(
    `# workload: heap-sort, array size=${size}, iterations per worker=${iters}`
  );
  console.log(
    `# threads=${threads}  seed=${
      seed ?? "(random)"
    ...
    workers.push(
      new Promise((resolve, reject) => {
        const w = new Worker(__filename, {
          workerData: { iters, size, seed: seed != null ? seed + i : null },
        });
  ...
  console.log(`Array size        : ${numberFormatter.format(size)}`);
  console.log(`Wall time (ms)    : ${numberFormatter.format(wallMs)}`);
  console.log(
    `Throughput        : ${numberFormatter.format(
      elemsPerSec
    )} elements/s (aggregate)`
  );
  console.log(
    `Per-iteration ms  : mean=${s.mean.toFixed(3)}  std=${s.std.toFixed(
      3
    )}  min=${s.min.toFixed(3)}  max=${s.max.toFixed(3)}`
  );
  console.log(
    "Note: per-iteration values are distribution of all worker iterations."
  );
...
# 8 threads
time node bench.js --iters=200 --threads=8 --size=1000000

# 4 threads
time node bench.js --iters=400 --threads=4 --size=1000000

# 1 thread
time node bench.js --iters=1600 --threads=1 --size=1000000

## yarn build simulation
time node bench.js --iters=800 --threads=2 --size=750000

## Unlimited (based on yarn test)

# 13600k
time node bench.js --iters=80 --threads=20 --size=1000000

# Air M4 and Pro M4 base
time node bench.js --iters=160 --threads=10 --size=1000000

# M4 Pro
time node bench.js --iters=133 --threads=12 --size=1000000
```
</main>
</Layout>
